...
public class LinePath implements Paintable {
    ...
    public ArrayList<PathPart> evaluatePath(double startX, double startY, double endX, double endY) {
        ...
        if (this.map.getIntersection(new PaddedLine(endX - 2, endY - 2, endX + 2, endY + 2, 4)) != null) {
            return paths;
        }

        PaddedLine path = this.createPaddedPath(startX, startY, endX, endY);
        SolidObject intersection = this.map.getIntersection(path);
        while (intersection != null) {
            PaddedLine pathToIntersection = this.createPaddedPath(path.getStartX(), path.getStartY(), intersection.getCenterX(), intersection.getCenterY());

            paths.addAll(this.handleIntersection(pathToIntersection));

            PaddedLine lastPath = paths.get(paths.size() - 1).getLine();
    ...
    private ArrayList<PathPart> handleIntersection(PaddedLine path) {
        ArrayList<PathPart> paths = new ArrayList<>();
        SolidObject intersection = this.map.getIntersection(path);
        while (intersection != null) {
    ...
    private PaddedLine createPaddedPath(double startX, double startY, double endX, double endY) {
        return new PaddedLine(startX, startY, endX, endY, this.robotControl.getRobotBounds().getWidth());
    }

    private PaddedLine handleIntersectionWithWall(PaddedLine paddedLine, Map map) {
        double distanceToWall = map.getDistanceToClosestIntersectionWithWall(paddedLine);
        if (distanceToWall != -1) {
            paddedLine.setDistance(distanceToWall);
